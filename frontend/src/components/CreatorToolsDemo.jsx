...
const tabs = [
  ['accounts', '계정 추천', Search], ['keywords', '트렌드 키워드', TrendingUp], ['link', '링크 분석', Link],
  ['video', '컷편집·자막', Scissors], ['feedback', '콘텐츠 피드백', MessageSquare],
]
const titles = { accounts: '내 콘텐츠에 맞는 레퍼런스', keywords: '다음 콘텐츠의 키워드 찾기', link: '릴스 원문 대본 추출', video: '영상은 간결하게, 자막은 정확하게', feedback: '게시 전, 한 번 더 완성도 높이기' }
...
const toolNotes = ['주제·타깃에 맞는 계정 비교', '넓은 키워드에서 기획까지', '원문 추출·한국어 해석', '삭제 구간과 자막 직접 편집', '영상·캡션의 개선점 확인']
...
export default function CreatorToolsDemo() {
  ...
  return <main className={`creator-tools demo-tools${activeView==='tools'&&mode==='video'&&step==='results'&&videoDetailed?' demo-video-editing':''}${mode==='video'&&step==='results'?' demo-video-workspace':''}`} data-theme={theme}>
    ...
    <div className="ct-workspace">
      ...
      <header className="ct-header"><div><div className="ct-eyebrow">WORKSPACE / CREATOR STUDIO</div><h1>{activeView==='board'?'제작 보드':'크리에이터 스튜디오'}</h1><p>레퍼런스 발견부터 영상 편집과 게시 전 점검까지.</p></div><button className="ct-primary" onClick={()=>navigate('link')}><Plus size={17}/>새 작업</button></header>
      <div className="demo-metrics"><div><span>이번 작업 목표</span><strong>{goal}</strong><small>콘텐츠 → 신뢰 → 전환</small></div><div><span>저장한 레퍼런스</span><strong>{saved.length}<small>개</small></strong><small>비교할 계정 모아보기</small></div><div><span>제작할 콘텐츠</span><strong>{board.length}<small>개</small></strong><button onClick={()=>setActiveView('board')}>제작 보드 열기<ChevronRight size={13}/></button></div><div><span>현재 작업 주제</span><strong className="demo-metric-topic">{topic}</strong><small>예시 소재 · 정리 / 라이프스타일</small></div></div>
