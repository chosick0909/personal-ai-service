...
export default function CreatorMediaEditor({ media, onSave, onRender, onRefresh, busy, renderEnabled }) {
  ...
  function edit(next) {
    video.current?.pause(); stop.current = null; setError(''); setMarks({start:null,end:null})
  ...
  function cutRange() {
    const next = removeRange(draft.clips, marks.start, marks.end, uid)
    if (durationOf(next) < .5) return setError('최소 0.5초의 영상을 남겨주세요.')
    edit({ ...draft, clips: next })
  }
  function applyCuts() {
    let clips = [{ id: uid(), assetId: 'original', start: 0, end: Number(media.durationSeconds) }]
