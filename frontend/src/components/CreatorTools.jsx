...
function CreatorWorkspace() {
  ...
  return <main className="creator-tools ct-live" data-theme={theme}>
    ...
    <form onSubmit={submit} className="ct-form">
      {mode === 'reference-accounts' && <div className="ct-fields ct-fields-two"><SelectField label="카테고리" value={brief.category} onChange={category=>formChanged({category})} required options={[["","카테고리 선택"],...referenceCategories.map(category=>[category,category])]}/><SelectField label="얼굴 노출" value={brief.faceVisibility} onChange={faceVisibility=>formChanged({faceVisibility})} options={[["any","상관없음"],["visible","얼굴 자주 등장"],["mixed","얼굴 일부 등장"],["hidden","얼굴 비공개"]]}/><SelectField label="계정 규모" value={brief.accountSize} onChange={accountSize=>formChanged({accountSize})} options={[["any","상관없음"],["10k_50k","1만~5만"],["50k_200k","5만~20만"],["over_200k","20만 이상"]]}/><SelectField label="콘텐츠 언어" value={brief.contentLanguage} onChange={contentLanguage=>formChanged({contentLanguage})} options={[["any","상관없음"],["ko","한국어"],["en","영어"],["ja","일본어"]]}/></div>}
