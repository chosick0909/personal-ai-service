...
const stages = { verifying_public_accounts: '공개 프로필·최근 게시물 확인 중', verifying_reach: '50만 이상 조회 콘텐츠 확인 중', reviewing_content: '영상 표본·캡션 분석 중', queued: '작업 대기 중', finding_accounts: '카테고리 상위 계정 검색 중', ranking_accounts: '선택 조건 적합도 비교 중',
...
function AccountResults({ result, preferences, onPreference }) {
  ...
  return <section className="ct-results">
    ...
    {accounts.length === 0 && <div className="ct-empty"><Search size={28}/><strong>팔로워 범위와 50만 이상 조회 콘텐츠가 모두 확인된 계정을 찾지 못했습니다.</strong></div>}
...
function CreatorWorkspace() {
  ...
  return <main className="creator-tools ct-live" data-theme={theme}>
    ...
    <form onSubmit={submit} className="ct-form">
      ...
      {mode==='reference-accounts' && <p className="ct-muted">공개 계정의 주제, 전달 방식, 최근 활동을 비교해 내 콘텐츠 방향을 살펴봅니다. 선택한 팔로워 범위와 수집된 공개 게시물 중 조회수 50만 이상 콘텐츠 보유 여부는 필수 조건입니다.</p>}
