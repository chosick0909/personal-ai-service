...
const tools = [
  { id: 'reference-accounts', label: '레퍼런스 계정', title: '내 콘텐츠에 맞는 레퍼런스', note: '카테고리·조건에 맞는 공개 계정 비교', icon: Search },
...
const stages = { verifying_public_accounts: '공개 프로필·최근 게시물 확인 중', reviewing_content: '영상 표본·캡션 분석 중', queued: '작업 대기 중', finding_accounts: '카테고리 상위 계정 검색 중', ranking_accounts: '선택 조건 적합도 비교 중',
...
function AccountResults({ result, preferences, onPreference }) {
  ...
  return <section className="ct-results">
    {result.message && <p className="ct-muted">{result.message}</p>}
    {(result.accounts || []).filter((a) => !excluded.has(a.username)).map((account) => <article className="ct-account" key={account.username}>
      <header><a href={safeInstagramHref(account.profileUrl) || undefined} target="_blank" rel="noreferrer">@{account.username} <ExternalLink size={15} /></a><span>선택 조건 적합도 {Number.isFinite(account.matchScore)?account.matchScore:'확인되지 않음'}</span>
        ...
        <IconButton title="추천에서 제외" onClick={() => onPreference(account.username, 'excluded')}><X size={18} /></IconButton></header>
      <ul>{account.reasons.map((reason, i) => <li key={i}>{reason}</li>)}</ul>
      {account.referencePoints.map((point, i) => <p key={i}>{point}</p>)}
      {account.relaxedConditions?.length > 0 && <p className="ct-muted">가까운 추천 · {account.relaxedConditions.join(' · ')}</p>}
      <div className="ct-tags"><span>얼굴 노출 {({visible:'확인됨',hidden:'비공개형',mixed:'일부 등장',unknown:'확인되지 않음'})[account.faceVisibility] || '확인되지 않음'}</span><span>언어 {({ko:'한국어',en:'영어',ja:'일본어',unknown:'확인되지 않음'})[account.contentLanguage] || '확인되지 않음'}</span></div>
      <div className="ct-links">{account.exampleMedia.map((media, i) => safeInstagramHref(media.permalink) ? <a key={i} href={safeInstagramHref(media.permalink)} target="_blank" rel="noreferrer">참고 게시물 {i + 1} <ExternalLink size={14} /></a> : null)}</div>
      <div className="ct-tags"><span>팔로워 {Number.isFinite(account.followers) ? account.followers.toLocaleString() : '확인되지 않음'}</span><span>계정 게시물 {Number.isFinite(account.postsCount) ? account.postsCount.toLocaleString() : '확인되지 않음'}</span></div>
      <small className="ct-muted">출처: {account.source || '확인되지 않음'} · 확인된 최근 게시물: {checkedAt(account.lastActiveAt)} · 조회: {checkedAt(account.metricsAsOf)}{!account.engagementAvailable && ' · 반응 효율 수치 확인 불가'}</small>
    </article>)}
  </section>
}
...
function CreatorWorkspace() {
  ...
  const [brief, setBrief] = useState({ category: '', region: 'KR', faceVisibility: 'any', contentFormat: 'any', accountSize: 'any', recentActivity: 'any', contentLanguage: 'ko',
    ...
    try {
      ...
      if (['media-analyze','feedback'].includes(mode)) {
        ...
        const discoveryBrief = mode === 'reference-accounts' ? { category:brief.category, faceVisibility:brief.faceVisibility,
          contentFormat:brief.contentFormat, accountSize:brief.accountSize, recentActivity:brief.recentActivity, contentLanguage:brief.contentLanguage }
  ...
  return <main className="creator-tools ct-live" data-theme={theme}>
    ...
    <header className="ct-header"><div><div className="ct-eyebrow">WORKSPACE / CREATOR STUDIO <span className="ct-badge">Beta</span></div><h1>크리에이터 스튜디오</h1><p>레퍼런스 발견부터 영상 편집과 게시 전 점검까지.</p></div></header>
    ...
    }<button type="button" className="ct-mobile-history" aria-current={historyMode?'page':undefined} onClick={()=>changeMode(historyMode?'reference-accounts':'recent-jobs')}><Clock3 size={17}/>{historyMode?'콘텐츠 도구로 돌아가기':`최근 작업 ${jobs.length}`}</button>
    ...
    <div className="ct-section-heading"><h2><SlidersHorizontal size={19} />{isMedia ? '원본 영상' : mode === 'import-link' ? '분석할 레퍼런스' : '콘텐츠 조건'}</h2><span className="ct-badge ct-badge-neutral">{isMedia ? 'MP4 · MOV · WebM' : 'Instagram'}</span></div>
    <form onSubmit={submit} className="ct-form">
      {mode === 'reference-accounts' && <div className="ct-fields ct-fields-two"><SelectField label="카테고리" value={brief.category} onChange={category=>formChanged({category})} required options={[["","카테고리 선택"],...referenceCategories.map(category=>[category,category])]}/><SelectField label="얼굴 노출" value={brief.faceVisibility} onChange={faceVisibility=>formChanged({faceVisibility})} options={[["any","상관없음"],["visible","얼굴 자주 등장"],["mixed","얼굴 일부 등장"],["hidden","얼굴 비공개"]]}/><SelectField label="콘텐츠 형식" value={brief.contentFormat} onChange={contentFormat=>formChanged({contentFormat})} options={[["any","상관없음"],["talking","말하는 영상"],["tutorial","사용법·시연"],["vlog","브이로그"],["before_after","전후 비교"],["review","제품 리뷰"],["text","텍스트 중심"]]}/><SelectField label="계정 규모" value={brief.accountSize} onChange={accountSize=>formChanged({accountSize})} options={[["any","상관없음"],["under_10k","1만 미만"],["10k_50k","1만~5만"],["50k_200k","5만~20만"],["over_200k","20만 이상"]]}/><SelectField label="최근 활동" value={brief.recentActivity} onChange={recentActivity=>formChanged({recentActivity})} options={[["any","상관없음"],["7d","7일 이내"],["30d","30일 이내"],["90d","90일 이내"]]}/><SelectField label="콘텐츠 언어" value={brief.contentLanguage} onChange={contentLanguage=>formChanged({contentLanguage})} options={[["any","상관없음"],["ko","한국어"],["en","영어"],["ja","일본어"]]}/></div>}
      ...
      {mode==='reference-accounts' && <p className="ct-muted">선택 카테고리의 공개 계정을 확인합니다. 조건이 완전히 일치하지 않으면 가장 가까운 계정과 차이를 함께 표시합니다.</p>}
      {mode==='import-link' && <p className="ct-muted">사전 등록 계정 제한 없이 공개 Instagram 릴스·영상 링크를 입력할 수 있습니다. 수집 불가 시 파일·대본으로 이어서 작업하세요.</p>}
    ...
    {mode === 'import-link' && <p className="ct-muted">현재 Instagram 공개 개별 영상 링크만 지원합니다. 국내·해외 언어를 감지해 한국어로 번역합니다. 다른 플랫폼, 비공개·삭제·로그인 필요 영상은 파일 또는 대본 입력을 사용해주세요.</p>}
    {mode === 'import-link' && <button type="button" onClick={() => changeMode('media-analyze')}>파일 업로드로 전환</button>}
...
function FeedbackResults({ result, media, onRefresh }) {
  ...
  return <section className="ct-results"><h2>영상·캡션 피드백</h2>{media?.previewUrl && <div className="ct-feedback-preview"><video ref={player} src={media.previewUrl} controls playsInline onTimeUpdate={()=>{if(stop.current!==null && player.current.currentTime>=stop.current){player.current.pause();stop.current=null}}}/><button type="button" onClick={onRefresh}>미리보기 링크 갱신</button></div>}<p className="ct-muted">{result.scope}</p><p className="ct-muted">조회수·매출을 예측하거나 보장하지 않습니다.</p>{result.findings.map((f,i)=><article className="ct-account" key={i}><div className="ct-row"><h3>{names[f.area]}</h3><span className="ct-badge">{evidence[f.evidence]}</span><span>{f.start===null?'구간 확인 없음':`${f.start.toFixed(2)}–${f.end.toFixed(2)}초`}</span>{f.start!==null && media?.previewUrl && <button type="button" onClick={()=>{player.current.currentTime=f.start;stop.current=f.end;void player.current.play().catch(()=>{})}}>문제 구간 재생</button>}</div><p><strong>{f.problem}</strong></p><p>{f.reason}</p><blockquote>수정 예시: {f.example}</blockquote></article>)}</section>
}
