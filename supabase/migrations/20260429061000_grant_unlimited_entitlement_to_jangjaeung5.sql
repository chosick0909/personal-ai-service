-- Grant the owner account unlimited access without an expiration date.
...
begin
  select id
    into v_user_id
  from auth.users
  where lower(email) = 'jangjaeung5@gmail.com'
  order by created_at asc
  limit 1;

  if v_user_id is null then
    raise notice 'Skipped unlimited entitlement grant: auth user jangjaeung5@gmail.com not found.';
    return;
  end if;
