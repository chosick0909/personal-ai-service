export const VARIATION_CONFIGS = [
  {
    label: 'A안',
    angle: '원본형',
    retrievalHint:
      '레퍼런스 문장 수, 전개 순서, 길이감, 역할 순서, 원본 흐름 유지, 소재 치환',
  },
  {
    label: 'B안',
    angle: '대화형',
    retrievalHint:
      '레퍼런스 구조 유지, 자연스러운 말투, 대화체, 흐름 보정, 어색함 제거',
  },
  {
    label: 'C안',
    angle: '후킹형',
    retrievalHint:
      '레퍼런스 구조 유지, 후킹 강화, 욕구 자극, CTA 강화, 저장/상담/구매 전환',
  },
]

export const CATEGORY_ANCHOR_TERMS = {
  뷰티: ['피부', '스킨케어', '메이크업', '화장', '제품', '루틴'],
  '운동/헬스': ['운동', '헬스', '피트니스', '자세', '근력', '루틴'],
  육아: ['육아', '아이', '부모', '월령', '아기', '양육'],
  반려동물: ['반려동물', '강아지', '고양이', '보호자', '사료', '간식'],
  살림: ['살림', '정리', '수납', '청소', '주방', '생활'],
  자기계발: ['자기계발', '습관', '생산성', '성장', '목표', '실행'],
  패션: ['패션', '코디', '스타일', '의류', '착장', '핏'],
  AI: ['AI', '자동화', '프롬프트', '업무', '툴', '생산성'],
  '전문직(회사홍보)': ['전문성', '상담', '고객', '사례', '브랜딩', '서비스'],
  재테크: ['재테크', '지출', '예산', '투자', '자산', '가계부'],
  여행: ['여행', '일정', '코스', '숙소', '항공', '예산'],
  요리: ['요리', '레시피', '재료', '식단', '조리', '주방'],
  '테크 가젯': ['가젯', '디바이스', '리뷰', '비교', '사용성', '기능'],
  멘탈케어: ['멘탈', '감정', '스트레스', '회복', '루틴', '심리'],
  교육: ['교육', '학습', '강의', '개념', '설명', '이해'],
  기타: [],
}

export const CREATOR_BUSINESS_PROOF_PATTERN =
  /(팔로워|수강생|강의|무료\s*강의|알고리즘|노출|계정\s*구조|AI\s*자동화|잠재\s*고객|리드|문의\s*전환|매출|수익화\s*모델|월\s*\d+[^\s]{0,4}(?:원|만원|억))/i

export const DOMAIN_EVIDENCE_PROFILES = {
  health_fitness: {
    preferred:
      '루틴 지속 가능성, 수행 변화, 자세/자극 체감, 초보자 실수 교정, before-after 과정, 습관 변화, 체력/컨디션 개선',
    avoid:
      '팔로워 성장, 수강생 매출, 강의 전환, AI 자동화, 알고리즘/노출 구조 같은 크리에이터 비즈니스 근거',
  },
  뷰티: {
    preferred: '피부 고민 변화, 사용감, 전후 차이, 성분/루틴 체감, 지속력, 들뜸/트러블 개선 과정',
    avoid: '수강생·강의·매출·팔로워 성장 같은 사업형 사례',
  },
  육아: {
    preferred: '양육 과정의 편의성, 아이 반응, 부모의 불안 해소, 실제 사용 경험, 반복 사용 만족감',
    avoid: '계정 성장, 강의 판매, 수강생 수익 사례',
  },
  반려동물: {
    preferred: '반려동물 반응, 행동 변화, 보호자 편의, 건강/위생 체감, 실제 급여/사용 경험',
    avoid: '크리에이터 강의·수강생·팔로워·매출형 사례',
  },
  살림: {
    preferred: '정리 전후 차이, 시간 절약, 공간 효율, 생활 편의, 반복 사용성, 계절성 문제 해결',
    avoid: '강의/수강생/노출/계정 구조형 근거',
  },
  자기계발: {
    preferred: '실행 습관 변화, 몰입도, 체크리스트 적용, 생산성 체감, 꾸준함 유지, 목표 달성 과정',
    avoid: '뜬금없는 팔로워·수강생·월매출 서사',
  },
  패션: {
    preferred: '핏 변화, 체형 보정, 코디 편의, 상황별 스타일링 효과, 착용감, 전후 인상 차이',
    avoid: '강의 판매/수강생 매출/알고리즘 서사',
  },
  AI: {
    preferred: '시간 절약, 자동화, 실무 효율, 정확도 향상, 도구 비교, 반복 업무 절감',
    avoid: '사용자 도메인과 무관한 화장품/운동/육아 식 사례',
  },
  '전문직(회사홍보)': {
    preferred: '상담 전환, 실제 사례, 신뢰 형성, 전문성, 문의 맥락, 고객 문제 해결 과정',
    avoid: '뜬금없는 뷰티/육아/여행형 체험담',
  },
  재테크: {
    preferred: '지출 구조, 원칙, 리스크 관리, 절세/예산 체감, 실행 순서, 돈이 새는 포인트',
    avoid: '뷰티/운동/수강생 매출형 크리에이터 사례',
  },
  여행: {
    preferred: '일정 편의, 비용 절약, 동선 최적화, 장소 만족감, 실제 코스 체감, 시행착오 절감',
    avoid: '강의/수강생/노출 알고리즘형 근거',
  },
  요리: {
    preferred: '조리 편의, 맛/식감 체감, 재료 절약, 실패 감소, 시간 단축, 따라 하기 쉬움',
    avoid: '크리에이터 비즈니스 성과형 근거',
  },
  '테크 가젯': {
    preferred: '실사용 편의, 기능 차이, 생산성 개선, 세팅 간결함, 비교 리뷰, 작업 흐름 개선',
    avoid: '화장품/수강생/월수익형 서사',
  },
  멘탈케어: {
    preferred: '감정 안정, 스트레스 완화, 수면/회복 체감, 작은 습관 변화, 공감과 안도감',
    avoid: '강의 판매나 월매출 같은 과한 사업형 증거',
  },
  교육: {
    preferred: '이해도 상승, 학습 효율, 설명력, 사례 기반 학습, 수업/강의 맥락, 개념 적용',
    avoid: '도메인과 무관한 화장품/운동/살림 체험담',
  },
  기타: {
    preferred: '현재 계정의 타겟 문제, 사용 경험, 적용 전후 차이, 신뢰를 주는 현실적 사례',
    avoid: '레퍼런스 원문의 팔로워/수강생/강의/월매출 같은 그대로의 사업형 사례',
  },
}

export const CATEGORY_PLAYBOOKS = {
  '운동/헬스': {
    meta: {
      label: '운동/헬스',
      aliases: ['운동', '헬스', '피트니스', 'fitness', '자세', '필라테스', '홈트'],
    },
    uiCopy: {
      insight: '운동 콘텐츠는 일반 정보보다 “내가 지금 뭘 잘못하고 있는지”를 먼저 보여줄 때 몰입이 더 잘 붙습니다.',
      hookAiRule: '실수 지적 뒤에 바로 고칠 수 있는 한 가지 동작을 주고, 바뀌는 체감까지 짧게 연결하는 편이 좋습니다.',
    },
    promptRules: {
      hard: ['일반론만 길게 설명하지 마라', '근거 없이 공포를 조장하지 마라', '몸을 비하하는 표현을 쓰지 마라'],
      soft: ['잘못된 동작이나 실수 포인트를 먼저 짚어라', '바로 따라 할 수 있는 교정 1개를 줘라', '교정 뒤 달라지는 체감을 짧게 보여줘라'],
    },
    generationHints: {
      hookTypes: ['실수 지적형', '자세 교정형', '전후 체감형'],
      winningStructures: ['실수 지적 → 교정 → 체감 → 저장/상담'],
      ctaTypes: ['저장', '댓글', 'DM', '체험권', '상담 예약'],
      tones: ['코치형', '단호형', '근거형'],
      visualOpeners: ['잘못된 자세', '통증 부위 표시', '전후 동작 비교'],
    },
    modes: {
      awareness: {
        emphasis: ['실수 자각', '자기진단', '저장 가치'],
      },
      conversion: {
        emphasis: ['교정 체감', '상담 연결', '체험/프로그램 안내'],
      },
    },
    validation: {
      forbiddenStarts: ['몸매가 별로', '이거 안 하면 망가집니다', '무조건 아프게 됩니다'],
      requiredConversionElements: ['다음 행동', '저장/DM/상담 연결'],
    },
  },
  육아: {
    meta: {
      label: '육아',
      aliases: ['육아', '엄마', '맘', '월령', '아기', '부모'],
    },
    uiCopy: {
      insight: '육아 콘텐츠는 불안을 세게 찌르기보다 공감 뒤에 안심을 주는 흐름이 더 자연스럽습니다.',
      hookAiRule: '훈계보다 “나도 그랬다 → 괜찮다 → 이렇게 해보자” 흐름으로 시작하면 신뢰가 높아집니다.',
    },
    promptRules: {
      hard: ['훈계 톤으로 시작하지 마라', '공포를 과장하지 마라', '부모 탓으로 돌리지 마라'],
      soft: ['공감 후 안심 흐름을 우선하라', '월령/상황 맥락을 자연스럽게 넣어라', '작고 바로 가능한 해결을 제시하라'],
    },
    generationHints: {
      hookTypes: ['공감형', '불안해소형', '월령/상황형'],
      winningStructures: ['공감 → 상황 → 안심 → 해결 → 저장/DM'],
      ctaTypes: ['저장', '댓글', 'DM', '자료 링크'],
      tones: ['엄마 친구형', '안심형', '부드러운 전문가형'],
      visualOpeners: ['아이 행동 장면', '엄마 표정', '육아 난장판'],
    },
    modes: {
      awareness: {
        emphasis: ['공감', '현실감', '정서 진입'],
      },
      conversion: {
        emphasis: ['월령별 문제 해결', '자료/제품 연결', '안심 후 행동 유도'],
      },
    },
    validation: {
      forbiddenStarts: ['엄마가 잘못', '이거 안 하면 큰일', '지금 당장 안 하면'],
      requiredConversionElements: ['다음 행동', '저장/DM/자료 연결'],
    },
  },
  살림: {
    meta: {
      label: '살림',
      aliases: ['살림', '정리', '수납', '청소', '생활'],
    },
    uiCopy: {
      insight: '살림 콘텐츠는 정리 원리보다 전후 변화와 시간 절약이 먼저 보여야 반응이 잘 붙는 편입니다.',
      hookAiRule: '첫 장면에서 “이렇게 하면 더 편해진다”는 결과를 보여주고, 설명은 뒤로 미루는 편이 좋습니다.',
    },
    promptRules: {
      hard: ['정리 철학부터 길게 설명하지 마라', '과정을 너무 길게 끌지 마라', '제품만 먼저 들이밀지 마라'],
      soft: ['전후 변화 폭을 먼저 보여줘라', '시간 절약이나 생활 편의를 자연스럽게 넣어라', '생활 맥락에서 바로 써먹는 느낌으로 써라'],
    },
    generationHints: {
      hookTypes: ['시간절약형', '전후비교형', '꿀팁형'],
      winningStructures: ['불편 상황 → 해결 도구 → 전후 차이 → CTA'],
      ctaTypes: ['저장', '제품 링크', '댓글 키워드', '구매'],
      tones: ['친근형', '실용형', '빠른 설명형'],
      visualOpeners: ['전후 비교', '지저분한 장면', '정리된 결과 컷'],
    },
    modes: {
      awareness: {
        emphasis: ['불편 공감', '전후 차이', '저장 가치'],
      },
      conversion: {
        emphasis: ['시간 절약', '제품 효용', '짧은 CTA'],
      },
    },
    validation: {
      forbiddenStarts: ['정리의 철학은', '오늘은 원리부터', '이 제품 무조건 사세요'],
      requiredConversionElements: ['해결 결과', '댓글/링크/구매 CTA'],
    },
  },
  재테크: {
    meta: {
      label: '재테크',
      aliases: ['재테크', '투자', '자산', '부업', '돈관리'],
    },
    uiCopy: {
      insight: '재테크 콘텐츠는 자극보다 신뢰가 우선입니다. 손실 회피와 판단 기준을 짚는 흐름이 더 안전하고 설득력 있습니다.',
      hookAiRule: '수익 약속 대신 “이 실수 때문에 돈이 샌다”처럼 판단 기준과 리스크 회피 중심으로 전개하는 편이 좋습니다.',
    },
    promptRules: {
      hard: ['수익을 보장하는 표현을 쓰지 마라', '과장된 숫자 약속을 하지 마라', '리스크를 숨기지 마라'],
      soft: ['판단 기준을 먼저 제시하라', '실수 회피형 메시지로 전개하라', 'CTA는 자료/진단처럼 낮은 허들로 설계하라'],
    },
    generationHints: {
      hookTypes: ['손실회피형', '실수 지적형', '체크리스트형'],
      winningStructures: ['실수 → 손해 포인트 → 기준 제시 → 자료/상담 CTA'],
      ctaTypes: ['저장', '무료자료', '댓글 키워드', '뉴스레터', '상담'],
      tones: ['차분한 전문가형', '현실조언형'],
      visualOpeners: ['숫자 오버레이', '가계부/통장 화면', '체크리스트'],
    },
    modes: {
      awareness: {
        emphasis: ['실수 자각', '손실 회피', '저장 가치'],
      },
      conversion: {
        emphasis: ['무료자료', '진단 연결', '낮은 허들 CTA'],
      },
    },
    validation: {
      forbiddenStarts: ['월 300 자동수익', '무조건 돈 벌어요', '원금 보장'],
      requiredConversionElements: ['판단 기준', '자료/상담/다음 행동'],
    },
  },
  자기계발: {
    meta: {
      label: '자기계발',
      aliases: ['자기계발', '생산성', '습관', '루틴'],
    },
    uiCopy: {
      insight: '자기계발 콘텐츠는 큰 다짐보다 오늘 바로 할 수 있는 작은 실행으로 끝날 때 저장 가치가 높아집니다.',
      hookAiRule: '실패 원인 진단 후, 부담 없는 한 가지 행동으로 마무리하면 실행감이 살아납니다.',
    },
    promptRules: {
      hard: ['추상적인 동기부여만 반복하지 마라', '죄책감이나 압박 위주로 몰지 마라'],
      soft: ['자기진단 후 작은 실행으로 이어가라', '오해를 깨고 바로 할 행동을 제시하라', '저장하고 다시 볼 만한 구조로 써라'],
    },
    generationHints: {
      hookTypes: ['자기진단형', '실패 원인형', '작은 실행형'],
      winningStructures: ['문제 진단 → 오해 깨기 → 작은 실행 → 저장'],
      ctaTypes: ['저장', '댓글', '템플릿', '체크리스트', '뉴스레터'],
      tones: ['친구형', '코치형', '담백한 동기부여'],
      visualOpeners: ['체크리스트', '루틴 장면', '책상/노트 화면'],
    },
    modes: {
      awareness: {
        emphasis: ['자기반성', '공감', '저장 가치'],
      },
      conversion: {
        emphasis: ['템플릿', '체크리스트', '뉴스레터/강의 연결'],
      },
    },
    validation: {
      forbiddenStarts: ['의지가 약해서', '게으르니까', '당장 인생 바꿔'],
      requiredConversionElements: ['작은 실행', '저장/템플릿/다음 행동'],
    },
  },
  '전문직(회사홍보)': {
    meta: {
      label: '전문직(회사홍보)',
      aliases: ['전문직', '회사홍보', '브랜드', '서비스', '상담'],
    },
    uiCopy: {
      insight: '전문직 콘텐츠는 전문용어보다 고객이 이미 궁금해하는 문제를 쉽게 풀어줄 때 신뢰가 더 빨리 생깁니다.',
      hookAiRule: '전문성을 앞세우기보다 “내 상황도 해당되나?” 싶은 질문을 먼저 건 뒤, 쉬운 설명과 문의 흐름으로 이어가세요.',
    },
    promptRules: {
      hard: ['전문용어부터 꺼내지 마라', '결과를 과장 보장하지 마라', '공포 마케팅으로 몰지 마라'],
      soft: ['고객 고민을 먼저 말하라', '쉬운 설명 뒤에 신뢰 근거를 붙여라', '문의 행동을 자연스럽게 열어라'],
    },
    generationHints: {
      hookTypes: ['불안 해소형', '오해 교정형', 'FAQ형'],
      winningStructures: ['고민 제시 → 쉬운 설명 → 신뢰 근거 → 문의 CTA'],
      ctaTypes: ['상담 예약', 'DM', '전화', '프로필 링크', '체크리스트'],
      tones: ['신뢰형', '쉽게 설명하는 전문가형'],
      visualOpeners: ['전문가 얼굴', '설명 장면', '상담/사례 보드'],
    },
    modes: {
      awareness: {
        emphasis: ['오해 교정', '신뢰 형성', '질문 유도'],
      },
      conversion: {
        emphasis: ['문의 연결', '상담 CTA', '체크리스트'],
      },
    },
    validation: {
      forbiddenStarts: ['무조건 치료', '결과 보장', '겁부터 주는 표현'],
      requiredConversionElements: ['쉬운 설명', '문의/상담/다음 행동'],
    },
  },
  반려동물: {
    meta: {
      label: '반려동물',
      aliases: ['반려동물', '강아지', '고양이', '펫', '보호자'],
    },
    uiCopy: {
      insight: '반려동물 콘텐츠는 정보 설명보다 귀여움·공감·걱정을 먼저 만들고 들어갈 때 감정 반응이 더 큽니다.',
      hookAiRule: '“우리 애도 이러는데 이유가 뭐지?” 같은 보호자 공감으로 시작한 뒤 팁이나 제품으로 넘어가는 편이 자연스럽습니다.',
    },
    promptRules: {
      hard: ['감정을 자극하기 위해 불안이나 고통을 과장하지 마라', '제품을 노골적으로 먼저 밀지 마라'],
      soft: ['귀여움이나 공감 장면으로 먼저 진입하라', '보호자 시점의 감정을 살려라', '팁이나 제품은 뒤에서 자연스럽게 연결하라'],
    },
    generationHints: {
      hookTypes: ['공감형', '귀여움→정보형', '행동 이유형'],
      winningStructures: ['행동/감정 장면 → 이유 설명 → 팁/제품 → 저장/공유'],
      ctaTypes: ['저장', '공유', '제품 링크', '훈련 상담', '댓글'],
      tones: ['보호자형', '따뜻한 전문가형', '유머형'],
      visualOpeners: ['귀여운 장면', '이상 행동 장면', '보호자 리액션'],
    },
    modes: {
      awareness: {
        emphasis: ['귀여움', '공감', '행동 이유'],
      },
      conversion: {
        emphasis: ['행동 해결', '제품/상담 연결', '쉬운 다음 행동'],
      },
    },
    validation: {
      forbiddenStarts: ['방치하면 큰일', '너무 위험', '무조건 문제 행동'],
      requiredConversionElements: ['공감 진입', '팁/제품/상담 연결'],
    },
  },
  뷰티: {
    meta: {
      label: '뷰티',
      aliases: ['뷰티', 'beauty', '피부', '스킨케어', '메이크업', '화장품'],
    },
    uiCopy: {
      insight: '뷰티 콘텐츠는 제품 장점보다 “내 피부/메이크업에서 왜 이런 문제가 생기는지”를 먼저 짚을 때 반응이 더 잘 붙습니다.',
      hookAiRule: '고민 상황을 먼저 보여주고, 기준 하나와 사용 맥락을 붙인 뒤 전후 체감으로 짧게 닫는 편이 좋습니다.',
    },
    promptRules: {
      hard: ['효과를 보장하지 마라', '피부 고민을 비하하지 마라', '제품만 앞세워 광고처럼 쓰지 마라'],
      soft: ['피부/메이크업 고민을 먼저 짚어라', '사용 기준과 추천 대상을 구분하라', '전후 체감이나 사용감을 구체적으로 보여줘라'],
    },
    generationHints: {
      hookTypes: ['고민 진단형', '사용감 비교형', '전후 체감형'],
      winningStructures: ['고민 상황 → 원인/기준 → 사용 맥락 → 체감 → 저장/링크'],
      ctaTypes: ['저장', '제품 링크', '댓글 키워드', '루틴 공유'],
      tones: ['친구형', '꼼꼼한 리뷰형', '현실 조언형'],
      visualOpeners: ['피부/메이크업 전후', '제품 제형 컷', '사용 장면'],
    },
    modes: {
      awareness: {
        emphasis: ['고민 공감', '기준 제시', '전후 체감'],
      },
      conversion: {
        emphasis: ['추천 대상', '사용 맥락', '링크/구매 CTA'],
      },
    },
    validation: {
      forbiddenStarts: ['무조건 좋아집니다', '이거 하나면 해결', '피부가 최악이면'],
      requiredConversionElements: ['추천/비추천 대상', '저장/링크/다음 행동'],
    },
  },
  패션: {
    meta: {
      label: '패션',
      aliases: ['패션', 'fashion', '코디', '스타일', '의류', '착장', '핏'],
    },
    uiCopy: {
      insight: '패션 콘텐츠는 예쁜 옷 소개보다 “왜 내 착장이 어색해 보이는지”를 먼저 보여줄 때 저장과 공유가 잘 일어납니다.',
      hookAiRule: '체형이나 상황을 비하하지 않고, 비율·색·핏 기준 하나를 잡아 바로 바꿀 수 있는 조합으로 연결하세요.',
    },
    promptRules: {
      hard: ['체형을 비하하지 마라', '무조건 사야 한다는 식으로 몰지 마라', '추상적인 예쁨만 반복하지 마라'],
      soft: ['어색해 보이는 이유를 기준으로 설명하라', '상황별 착장 맥락을 넣어라', '한 가지 수정 포인트를 바로 제시하라'],
    },
    generationHints: {
      hookTypes: ['핏 진단형', '비율 교정형', '상황별 코디형'],
      winningStructures: ['어색한 포인트 → 기준 설명 → 수정 조합 → 저장/제품 CTA'],
      ctaTypes: ['저장', '제품 링크', '댓글 키워드', '코디 상담'],
      tones: ['스타일리스트형', '친구 조언형', '담백한 비교형'],
      visualOpeners: ['전후 착장 비교', '비율 표시', '아이템 조합 컷'],
    },
    modes: {
      awareness: {
        emphasis: ['어색함 진단', '비율/핏 기준', '저장 가치'],
      },
      conversion: {
        emphasis: ['아이템 조합', '상황별 추천', '제품/상담 연결'],
      },
    },
    validation: {
      forbiddenStarts: ['몸매가 문제', '무조건 촌스러움', '이 옷 안 사면'],
      requiredConversionElements: ['수정 기준', '저장/링크/상담 CTA'],
    },
  },
  AI: {
    meta: {
      label: 'AI',
      aliases: ['AI', 'ai', '자동화', '프롬프트', '챗GPT', 'gpt', '업무툴'],
    },
    uiCopy: {
      insight: 'AI 콘텐츠는 기능 나열보다 “이 작업이 어디서 막히는지”를 먼저 보여주고 바로 쓸 프롬프트나 흐름을 줄 때 반응이 좋습니다.',
      hookAiRule: '문제 상황을 짧게 잡고, 입력값·도구·결과물을 한 번에 보이게 하면 저장과 재사용으로 이어지기 쉽습니다.',
    },
    promptRules: {
      hard: ['AI가 전부 대신한다는 식으로 과장하지 마라', '도구명만 나열하지 마라', '실행 단계 없이 추상적으로 설명하지 마라'],
      soft: ['막히는 업무 상황을 먼저 제시하라', '바로 복사할 수 있는 프롬프트나 절차를 넣어라', '결과물 차이를 보여줘라'],
    },
    generationHints: {
      hookTypes: ['업무 병목형', '프롬프트 개선형', '전후 결과형'],
      winningStructures: ['막히는 작업 → 프롬프트/도구 → 결과 차이 → 저장/템플릿 CTA'],
      ctaTypes: ['저장', '템플릿 다운로드', '댓글 키워드', '뉴스레터', '강의'],
      tones: ['실무자형', '간결한 튜토리얼형', '자동화 코치형'],
      visualOpeners: ['Before/After 화면', '프롬프트 입력 컷', '업무 결과물'],
    },
    modes: {
      awareness: {
        emphasis: ['문제 상황', '결과 차이', '저장 가치'],
      },
      conversion: {
        emphasis: ['템플릿', '강의/자료 연결', '반복 사용성'],
      },
    },
    validation: {
      forbiddenStarts: ['AI가 다 해줍니다', '클릭 한 번으로 끝', '무조건 자동수익'],
      requiredConversionElements: ['실행 단계', '템플릿/저장/다음 행동'],
    },
  },
  여행: {
    meta: {
      label: '여행',
      aliases: ['여행', '트립', '일정', '코스', '숙소', '항공', '동선'],
    },
    uiCopy: {
      insight: '여행 콘텐츠는 감성보다 “가면 후회하는 포인트”나 “동선이 편해지는 기준”을 먼저 주면 저장 가치가 커집니다.',
      hookAiRule: '예쁜 장면만 보여주기보다 비용·동선·시간 중 하나의 판단 기준을 잡고, 바로 저장할 코스로 연결하세요.',
    },
    promptRules: {
      hard: ['장소를 과장해서 보장하지 마라', '광고처럼 칭찬만 나열하지 마라', '동선/비용 맥락 없이 추천만 하지 마라'],
      soft: ['후회 포인트나 선택 기준을 먼저 말하라', '일정/비용/동선 맥락을 넣어라', '저장 가능한 코스나 체크리스트로 마무리하라'],
    },
    generationHints: {
      hookTypes: ['후회 방지형', '동선 최적화형', '비용 절약형'],
      winningStructures: ['문제 상황 → 선택 기준 → 추천 코스 → 저장/링크 CTA'],
      ctaTypes: ['저장', '공유', '지도 링크', '일정표', '예약 링크'],
      tones: ['여행 친구형', '현실 팁형', '가이드형'],
      visualOpeners: ['지도/동선 컷', '장소 전경', '비용/일정 오버레이'],
    },
    modes: {
      awareness: {
        emphasis: ['후회 방지', '동선/비용 기준', '저장 가치'],
      },
      conversion: {
        emphasis: ['예약/링크 연결', '코스 자료', '명확한 다음 행동'],
      },
    },
    validation: {
      forbiddenStarts: ['무조건 인생 여행', '여기 안 가면 손해', '가격 보장'],
      requiredConversionElements: ['선택 기준', '저장/지도/예약 CTA'],
    },
  },
  요리: {
    meta: {
      label: '요리',
      aliases: ['요리', '레시피', '쿠킹', '식단', '재료', '조리'],
    },
    uiCopy: {
      insight: '요리 콘텐츠는 완성샷보다 실패를 줄이는 포인트나 “왜 맛이 달라지는지”를 먼저 보여줄 때 저장이 잘 됩니다.',
      hookAiRule: '재료보다 순서와 기준을 먼저 잡고, 따라 했을 때 달라지는 맛·시간·실패 감소를 짧게 연결하세요.',
    },
    promptRules: {
      hard: ['맛을 과장 보장하지 마라', '재료 나열만 길게 하지 마라', '과정을 불필요하게 늘리지 마라'],
      soft: ['실패 포인트를 먼저 짚어라', '순서와 기준을 명확히 말하라', '시간 절약이나 맛 차이를 구체적으로 보여줘라'],
    },
    generationHints: {
      hookTypes: ['실패 방지형', '시간 단축형', '맛 차이 기준형'],
      winningStructures: ['흔한 실패 → 핵심 기준 → 조리 순서 → 저장/재료 CTA'],
      ctaTypes: ['저장', '재료 링크', '댓글 레시피', '공유'],
      tones: ['친근한 주방형', '빠른 튜토리얼형', '현실 레시피형'],
      visualOpeners: ['완성샷', '실패/성공 비교', '조리 과정 클로즈업'],
    },
    modes: {
      awareness: {
        emphasis: ['실패 공감', '핵심 기준', '저장 가치'],
      },
      conversion: {
        emphasis: ['재료/도구 연결', '레시피 자료', '짧은 CTA'],
      },
    },
    validation: {
      forbiddenStarts: ['무조건 맛있음', '요리 못하면', '이거 안 사면 실패'],
      requiredConversionElements: ['조리 기준', '저장/재료/레시피 CTA'],
    },
  },
  교육: {
    meta: {
      label: '교육',
      aliases: ['교육', '학습', '강의', '수업', '공부', '개념'],
    },
    uiCopy: {
      insight: '교육 콘텐츠는 지식 설명보다 “왜 이해가 안 됐는지”를 먼저 풀어줄 때 시청자가 끝까지 따라옵니다.',
      hookAiRule: '막히는 지점을 짧게 짚고, 쉬운 비유나 예시 하나로 개념을 열어준 뒤 복습 행동으로 연결하세요.',
    },
    promptRules: {
      hard: ['어려운 용어부터 꺼내지 마라', '학습자를 탓하지 마라', '강의 홍보만 앞세우지 마라'],
      soft: ['막히는 지점을 먼저 공감하라', '쉬운 예시나 비유로 설명하라', '복습/자료/수업 CTA를 자연스럽게 넣어라'],
    },
    generationHints: {
      hookTypes: ['오해 교정형', '개념 쉽게 풀기형', '학습 막힘 해결형'],
      winningStructures: ['막힘/오해 → 쉬운 예시 → 핵심 정리 → 복습/자료 CTA'],
      ctaTypes: ['저장', '자료 다운로드', '댓글 질문', '수업 신청', '복습 체크리스트'],
      tones: ['쉬운 선생님형', '차분한 설명형', '격려형'],
      visualOpeners: ['칠판/노트', '개념도', 'Before/After 풀이'],
    },
    modes: {
      awareness: {
        emphasis: ['막힘 공감', '쉬운 이해', '저장 가치'],
      },
      conversion: {
        emphasis: ['자료/수업 연결', '복습 행동', '질문 유도'],
      },
    },
    validation: {
      forbiddenStarts: ['이것도 모르면', '공부 못하는 사람', '무조건 성적 오른다'],
      requiredConversionElements: ['쉬운 예시', '저장/자료/수업 CTA'],
    },
  },
}
